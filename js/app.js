...
async function uploadStudentsToSupabase() {
    ...
    for (const student of rayatData) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`فشل رفع الطالب: ${errorText}`);
        }
    }
}
...
document.getElementById('openBtn').onclick = async () => {
    ...
    try {
        ...
        console.error(error);
        alert('حدث خطأ أثناء فتح الحضور: ' + error.message);
    }
};
...
async function getAttendanceList() {
    ...
    if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`فشل جلب الحضور: ${errorText}`);
    }
...
async function finishAttendance() {
    try {
        ...
        const result = rayatData.map(student => {
            ...
            if (attendanceIds.includes(studentId)) {
                student['Attendance Indicator'] = 'Present';
            } else {
                student['Attendance Indicator'] = 'Absent';
            }
        ...
        const savedFinalAttendance = localStorage.getItem('finalAttendance');

        if (!savedFinalAttendance) {
            throw new Error('تعذر حفظ ملف الحضور النهائي');
        }
        ...
        if (!deleteResponse.ok) {
            ...
            throw new Error(`تم تجهيز الملف لكن تعذر إغلاق التسجيل: ${errorText}`);
        }

        localStorage.setItem('attendanceFinished', 'true');

        alert('تم إنهاء الحضور وتجهيز ملف الحضور بنجاح');

    } catch (error) {
        console.error(error);
        alert('حدث خطأ أثناء إنهاء الحضور: ' + error.message);
    }
}
